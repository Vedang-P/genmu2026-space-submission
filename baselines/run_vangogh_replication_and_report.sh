...
if [ -f "$ROOT/space-models/sd/space-Van_Gogh.safetensors" ]; then
  ...
  run_step "space_training" bash "$ROOT/training/run_space_training.sh"
fi
...
if [ "${FORCE_SPACE_REBUILD:-0}" != "1" ] && [ "${FORCE_SPACE_IMAGES:-0}" != "1" ] && has_expected_count "$ROOT/results/space/space-Van_Gogh"; then
  ...
  run_step "space_images" bash "$ROOT/inference/run_space_images.sh"
fi
...
run_step "evaluate_vangogh_strict" "$PYTHON_BIN" "$ROOT/evaluation/evaluate.py" --only-artist "Van Gogh" --method-keys "esd,uce,ca,space" --strict-validation --strict-artist "Van Gogh"
run_step "comparison_grid_vangogh" "$PYTHON_BIN" "$ROOT/evaluation/make_comparison_grids.py" --only-artist "Van Gogh" --methods "ESD-x,UCE,CA,SPACE"
