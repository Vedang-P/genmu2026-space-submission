...
if [ "$STRICT_EVAL" = "1" ]; then
  ...
  run_step "evaluate_strict" python "$ROOT/evaluation/evaluate.py" "${eval_args[@]}"
else
  run_step "evaluate_relaxed" python "$ROOT/evaluation/evaluate.py"
fi

run_step "comparison_grids" python "$ROOT/evaluation/make_comparison_grids.py"
