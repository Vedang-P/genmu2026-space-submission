...
"$PYTHON_BIN" "$ROOT/evaluation/evaluate.py" --only-artist "Van Gogh" --method-keys "esd,uce,ca" --strict-validation --strict-artist "Van Gogh"
"$PYTHON_BIN" "$ROOT/evaluation/make_comparison_grids.py" --only-artist "Van Gogh" --methods "ESD-x,UCE,CA"
