...
should_run "Kelly McKernan" && run_gen 1 6 "Kelly McKernan" "$WEIGHTS/uce-Kelly_McKernan.safetensors" "$ROOT/data/kelly_prompts.csv" "" "uce_images_kelly.log"
should_run "Van Gogh" && run_gen 2 6 "Van Gogh" "$WEIGHTS/uce-Van_Gogh.safetensors" "$ROOT/data/vangogh_prompts.csv" "" "uce_images_vangogh.log"
should_run "Tyler Edlin" && run_gen 3 6 "Tyler Edlin" "$WEIGHTS/uce-Tyler_Edlin.safetensors" "$ROOT/data/short_niche_art_prompts.csv" "Tyler Edlin" "uce_images_tyler_edlin.log"
should_run "Thomas Kinkade" && run_gen 4 6 "Thomas Kinkade" "$WEIGHTS/uce-Thomas_Kinkade.safetensors" "$ROOT/data/short_niche_art_prompts.csv" "Thomas Kinkade" "uce_images_thomas_kinkade.log"
should_run "Kilian Eng" && run_gen 5 6 "Kilian Eng" "$WEIGHTS/uce-Kilian_Eng.safetensors" "$ROOT/data/short_niche_art_prompts.csv" "Kilian Eng" "uce_images_kilian_eng.log"
should_run "Ajin: Demi Human" && run_gen 6 6 "Ajin: Demi Human" "$WEIGHTS/uce-Ajin_Demi_Human.safetensors" "$ROOT/data/short_niche_art_prompts.csv" "Ajin: Demi Human" "uce_images_ajin.log"
