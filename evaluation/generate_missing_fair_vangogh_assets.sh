...
resolve_space_ckpt() {
  ...
  echo "Could not find SPACE checkpoint. Checked:" >&2
  printf '  %s\n' "${candidates[@]}" >&2
  return 1
}
...
generate_fid_if_needed() {
  ...
  mkdir -p "$dir"
  local cmd=(
    "$PYTHON_BIN" "$ROOT/evaluation/generate_fid_samples.py"
...
SPACE_CKPT="$(resolve_space_ckpt)"
echo "SPACE checkpoint: $SPACE_CKPT"

progress_step 1 9 "Vanilla prompt-pair images"
generate_case_method_if_needed \
  "Vanilla" \
  "$ROOT/results/baseline/vangogh" \
  "$PYTHON_BIN" "$ROOT/inference/generate_esd_old.py" \
...
progress_step 2 9 "ESD-x prompt-pair images"
generate_case_method_if_needed \
  "ESD-x" \
...
progress_step 3 9 "UCE prompt-pair images"
generate_case_method_if_needed \
  "UCE" \
  "$ROOT/results/uce/uce-Van_Gogh" \
  "$PYTHON_BIN" "$ROOT/inference/generate_images.py" \
    --base_model "CompVis/stable-diffusion-v1-4" \
    --esd_path "$UCE_CKPT" \
    --prompts_path "$VANGOGH_CSV" \
...
progress_step 4 9 "SPACE prompt-pair images"
generate_case_method_if_needed \
  "SPACE" \
...
progress_step 5 9 "Concept Ablation prompt-pair images"
generate_ca_if_needed

progress_step 6 9 "Vanilla COCO-FID images"
generate_fid_if_needed "Vanilla" "$ROOT/results/fid/baseline"

progress_step 7 9 "ESD-x COCO-FID images"
generate_fid_if_needed "ESD-x" "$ROOT/results/fid/erased" "$ESD_PT"

progress_step 8 9 "UCE COCO-FID images"
generate_fid_if_needed "UCE" "$ROOT/results/fid/uce" "$UCE_CKPT"

progress_step 9 9 "SPACE COCO-FID images"
generate_fid_if_needed "SPACE" "$ROOT/results/fid/space" "$SPACE_CKPT"
...
if [ "$RUN_EVAL_AFTER" = "1" ]; then
  ...
  bash "$ROOT/evaluation/run_fair_vangogh_eval.sh"
else
  echo ""
  echo "Next:"
