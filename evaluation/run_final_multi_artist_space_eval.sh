...
TRAIN_SPACE="${TRAIN_SPACE:-1}"
FORCE_IMAGES="${FORCE_IMAGES:-0}"
FORCE_FID="${FORCE_FID:-0}"
SKIP_PREFLIGHT="${SKIP_PREFLIGHT:-0}"
METHOD_KEYS="${METHOD_KEYS:-esd,uce,space}"
FID_METHOD_KEYS="${FID_METHOD_KEYS:-esd,uce,space}"
GRID_METHODS="${GRID_METHODS:-ESD-x,UCE,SPACE}"
...
train_space_if_needed() {
  local artist="$1" slug="$2" csv="$3" filter="$4" concept="$5"
  local ckpt="space-models/sd/space-$slug.safetensors"
  if [ "$TRAIN_SPACE" != "1" ] || [ -f "$ckpt" ]; then
    return
  fi
  run_with_progress "SPACE training: $artist" "$PYTHON_BIN" training/space_sd.py \
...
for artist in "${selected_artists[@]}"; do
  ...
  space_ckpt="$(resolve_file "SPACE checkpoint for $artist" "$ROOT/space-models/sd/space-$SLUG.safetensors")"
  generate_space_method_images "SPACE $artist" "$space_ckpt" "$CSV" "$FILTER" "results/space" "space-$SLUG" "$samples"
  archive_checkpoint "$space_ckpt" "$artist" "space"
  ...
  generate_fid_if_needed "UCE $artist" "results/fid/$artist_slug/uce" \
    "$PYTHON_BIN" evaluation/generate_fid_samples.py \
      ...
      --num_inference_steps "$FID_STEPS" \
      --guidance_scale "$CFG"
  generate_fid_if_needed "SPACE $artist" "results/fid/$artist_slug/space" \
    "$PYTHON_BIN" evaluation/generate_fid_samples.py \
      --base_model CompVis/stable-diffusion-v1-4 \
      --esd_path "$space_ckpt" \
      --prompts_path data/coco_30k.csv \
...
for artist in "${selected_artists[@]}"; do
  "$PYTHON_BIN" evaluation/make_comparison_grids.py --only-artist "$artist" --methods "$GRID_METHODS"
...
lines = [
    ...
    "Methods: ESD-x, UCE, SPACE",
    f"Prompt-pair metrics: {os.environ.get('EXPECTED_PAIRS', '500')} matched pairs per artist/method.",
