...
def generate_images(
    ...
    total_expected = len(df) * num_samples
    completed = sum(
        1
        for _, row in df.iterrows()
        for i in range(num_samples)
        if os.path.exists(os.path.join(folder_path, f"{int(row.case_number)}_{i}.png"))
    )
    started = time.time()
    wandb_run = None
    if wandb_project and wandb_mode != "disabled":
        try:
            ...
            wandb_run = wandb.init(
                ...
                resume="allow",
                mode=wandb_mode,
                name=f"prompt-images-{model_name}",
                config={"model": model_name, "prompts": len(df), "samples_per_prompt": num_samples},
            )
        except Exception as exc:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--progress_path", default=None, help="append durable progress records to this JSONL file")
