#!/bin/bash
# Generate SPACE images for the current artist benchmark.
...
if [ "$ONLY_ARTIST" != "Van Gogh" ]; then
  echo "SPACE image generation is currently scoped to Van Gogh only. Received ONLY_ARTIST=$ONLY_ARTIST"
  exit 1
fi
