#!/bin/bash
# Train SPACE for the current artist benchmark.
...
if [ "$ONLY_ARTIST" != "Van Gogh" ]; then
  echo "SPACE is currently scoped to Van Gogh only. Received ONLY_ARTIST=$ONLY_ARTIST"
  exit 1
fi
...
cmd=(
  "$PYTHON_BIN" "$ROOT/training/space_sd.py"
