...
from core.esd_checkpoint import load_esd_checkpoint, save_esd_checkpoint
from core.esd_trainer import (
...
from core.sd_utils import esd_sd_call
...
def run_space_training(config: SPACEConfig) -> str:
    ...
    metrics_path = None
    if progress_dir:
        os.makedirs(os.path.join(progress_dir, "checkpoints"), exist_ok=True)
        metrics_path = os.path.join(progress_dir, "training_metrics.jsonl")
        with open(os.path.join(progress_dir, "run_config.json"), "w") as f:
            json.dump(
                {
                    "base_model_id": config.base_model_id,
                    "erase_concept": config.erase_concept,
    ...
    if config.wandb_project and config.wandb_mode != "disabled":
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"space-train-{config.exp_name or sanitize_checkpoint_name(config.erase_concept)}",
                config={
                    "artist": config.erase_concept,
                    "stage1_steps": config.stage1_steps,
                    "stage2_steps": config.stage2_steps,
                    "learning_rate": config.lr,
                    "lora_rank": config.lora_rank,
            ...
            print(f"W&B initialization failed; continuing with local progress logs: {exc}")
    pbar = tqdm(range(total_steps), desc="Training SPACE")
    ...
    for step in pbar:
        ...
        step_metrics = {
            ...
            "erase_loss": sum(erase_meter) / len(erase_meter),
            "style_loss": sum(style_meter) / len(style_meter),
            "content_loss": sum(content_meter) / len(content_meter),
            "image_loss": sum(image_meter) / len(image_meter),
            "preserve_loss": sum(preserve_meter) / len(preserve_meter),
            "adapter_norm": adapter_norm,
            "adapter_delta_norm": delta_norm,
            "adapter_update_norm": update_norm,
            "target_shift": last_target_shift,
        }
        if metrics_path:
        ...
        if wandb_run is not None:
            wandb_run.log(step_metrics, step=step + 1)
        if (
            progress_dir
            and config.checkpoint_interval > 0
            and (step + 1) % config.checkpoint_interval == 0
        ):
            snapshot = os.path.join(progress_dir, "checkpoints", f"space-step-{step + 1:04d}.safetensors")
            save_space_checkpoint(adapters, snapshot, build_space_metadata(config, selected_modules))
            print(f"Saved SPACE progress snapshot: {snapshot}")
    ...
    if wandb_run is not None:
        wandb_run.summary["final_loss"] = last_total_loss
        wandb_run.summary["final_update_norm"] = final_update_norm
